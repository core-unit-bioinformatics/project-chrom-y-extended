...
def main():
    ...
    concat["is_kmer_based"] = concat["plain_label"].apply(lambda n: n in kmer_based_labels or n == UNCERTAIN_SCRAMBLED)
    ...
    intervals = pr.from_dict(
        {
            ...
            "kmer_based": concat["is_kmer_based"].values,
    ...
    intervals = intervals.cluster(by=["kmer_based", "plain_label"], count=False, strand=False, slack=-3).df
    intervals.set_index("pd_idx", inplace=True)
    intervals.drop(["Chromosome", "Start", "End", "Strand", "kmer_based", "plain_label"], axis=1, inplace=True)
    ...
    concat.drop(
        [
            "Cluster", "source", "is_kmer_based", "plain_label", "top_enrich",
